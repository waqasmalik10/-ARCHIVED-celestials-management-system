...
const StoreTable = () => {
    const { storeList, isDeleteModal, setIsDeleteModal, handleStoreDelete } = useInventory()
    ...
    const navigate = useNavigate()
        ...
        const [allTableData, setAllTableData] = useState<StoreTableData[]>([]);
    ...
    const handleUpdate = (store: StoreTableData) => {
        navigate(`/inventory/update-store/${store.id}`);
    }
    const deleteModalRef = useRef<HTMLDivElement>(null);
    const modalRef = useRef<HTMLDivElement>(null)

    const handleDeleteStore = (store: StoreTableData) => {
        setIsDeleteModal(store)
        window.scrollTo(0, 0);
                ...
                <DeleteModal ref={deleteModalRef} closeButtonCLick={deleteModalClose}>
                    ...
                    <div className="flex flex-col gap-4 px-5 mb-5">
                        <p className="text-xl font-poppins text-white">
                            Store Id: <span className="font-bold">{isDeleteModal.id}</span>
                        </p>
                        <p className="text-xl font-poppins text-white">
                            Store Name: <span className="font-bold">{isDeleteModal.name}</span>
                        </p>
                        <p className="text-xl font-poppins text-white">
                            Company Id: <span className="font-bold">{isDeleteModal.companyId}</span>
                        </p>
                    </div>

                    <div className="border-t border-solid border-[#CDD6D7] py-6 px-5 flex justify-center">
                        <Button onClick={() => handleStoreDelete(isDeleteModal)} buttonClasses="flex justify-center mx-auto min-h-[64px] px-11 pb-[15px] pt-4 border border-solid border-[#CDD6D7] bg-[#283573] font-urbanist font-semibold text-xl leading-[160%] rounded-[15px] text-white" type="button">
                            Confirm Delete
                        </Button>
                    </div>
                </DeleteModal>
